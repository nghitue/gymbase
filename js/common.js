...
$( document ).ready(function() {
  $(".image-container").mouseover(function () {
    $(this).find('img').attr('src','./common_img/' + $(this).find('img').data("hover"));
  }).mouseout(function () {
    $(this).find('img').attr('src','./common_img/' + $(this).find('img').data("src"));
  });

  $(".list-hours").each(function( index ) {
    $(this).find('dl:gt(2)').hide();
    $(this).find('dl:gt(2)').addClass("hide");
  });

  $('.btn-group .next').click(function() {
    $(this).siblings().removeClass('disable');
    var check = $(this).closest('.calendar').find('.list-hours dl').last().hasClass('hide');
    if(!check){
      $(this).addClass("disable");
    }
    else{
      $(this).closest('.calendar').find('.list-hours dl').not('.hide').first().hide();
      $(this).closest('.calendar').find('.list-hours dl').not('.hide').first().addClass('hide');
      $(this).closest('.calendar').find('.list-hours dl').not('.hide').last().next().show();
      $(this).closest('.calendar').find('.list-hours dl').not('.hide').last().next().removeClass('hide');
    }
  });

  $('.btn-group .prev').click(function() {
    $(this).siblings().removeClass('disable');
    var check = $(this).closest('.calendar').find('.list-hours dl').first().hasClass('hide');
    if(!check){
      $(this).addClass("disable");
    }
    else{
      $(this).closest('.calendar').find('.list-hours dl').not('.hide').first().prev().show();
      $(this).closest('.calendar').find('.list-hours dl').not('.hide').first().prev().removeClass('hide');
      $(this).closest('.calendar').find('.list-hours dl').not('.hide').last().hide();
      $(this).closest('.calendar').find('.list-hours dl').not('.hide').last().addClass('hide');
    }
  });

  var h = $('.header_top').outerHeight();
  $('.bgmv').css('margin-top', h);
});
